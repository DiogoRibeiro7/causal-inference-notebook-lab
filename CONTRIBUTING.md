...
`pre-commit install` is worth the ten seconds: it runs ruff, mypy, and notebook
output stripping before each commit, so CI rarely tells you something you could
...
- **Notebooks are committed without outputs.** The `nbstripout` hook enforces
  this. Outputs are regenerated by `make notebooks`, the nightly workflow, and
